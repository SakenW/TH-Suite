...
class Config:
    ...
    @classmethod
    def find_database(cls, custom_path: str | None = None) -> Path | None:
        """查找数据库文件"""
        if custom_path:
...
async def execute_query(query: dict[str, str]):
    ...
    try:
        ...
        return JSONResponse(
            content={"columns": columns, "rows": rows, "count": len(rows)}
        )
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=Config.DEFAULT_PORT, help="服务器端口"
    )
    parser.add_argument("--host", default=Config.DEFAULT_HOST, help="服务器地址")
    parser.add_argument(
        "--mode", choices=["web", "cli"], default="web", help="运行模式"
    )
