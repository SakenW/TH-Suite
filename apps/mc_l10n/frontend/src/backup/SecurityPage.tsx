...
function SecurityPage() {
  ...
  const filteredIntegrityResults = integrityResults.filter(result => {
    ...
    if (searchTerm && !result.file_path.toLowerCase().includes(searchTerm.toLowerCase()))
      return false
    return true
  })
  ...
  const filteredLicenseRecords = licenseRecords.filter(record => {
    ...
    if (searchTerm && !record.asset_id.toLowerCase().includes(searchTerm.toLowerCase()))
      return false
    return true
  })
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Dialog open={!!selectedLog} onClose={() => setSelectedLog(null)} maxWidth='md' fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => setSelectedLog(null)}>{t('common.actions.close')}</Button>
        </DialogActions>
      </Dialog>
      ...
      >
                ...
                <Grid item xs={12}>
                  ...
                  <Typography variant='body2' sx={{ mb: 2 }}>
                    {selectedLicense.license_url ? (
                      <a
                        href={selectedLicense.license_url}
                        target='_blank'
        ...
        <DialogActions>
          <Button onClick={() => setSelectedLicense(null)}>{t('common.actions.close')}</Button>
        </DialogActions>
      </Dialog>
    </Box>
  )
