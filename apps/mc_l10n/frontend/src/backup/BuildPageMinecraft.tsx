...
export default function BuildPageMinecraft() {
  ...
  const handleSaveConfig = (config: BuildConfig) => {
    if (editingConfig) {
      setBuildConfigs(configs => configs.map(c => (c.id === config.id ? config : c)))
    } else {
  ...
  return (
    <Box sx={{ p: 3 }}>
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select defaultValue={editingConfig?.type || 'modpack'} label='类型'>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select defaultValue={editingConfig?.loader || 'forge'} label='加载器'>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                <Select defaultValue={editingConfig?.outputFormat || 'zip'} label='输出格式'>
            ...
            <Grid item xs={12}>
              <FormControlLabel
                control={<Switch defaultChecked={editingConfig?.includeDocs ?? true} />}
                label='包含文档'
              />
              <FormControlLabel
                control={<Switch defaultChecked={editingConfig?.validateTranslations ?? true} />}
                label='验证翻译'
              />
              <FormControlLabel
                control={<Switch defaultChecked={editingConfig?.optimizeAssets ?? false} />}
                label='优化资源'
              />
            </Grid>
          </Grid>
      ...
      >
          ...
          >
            {selectedTask?.logs.map((log, index) => (
              <Box key={index} sx={{ mb: 0.5 }}>
                <span style={{ color: '#00FF00' }}>[{new Date().toLocaleTimeString()}]</span> {log}
              </Box>
            ))}
        ...
        <DialogActions>
          <MinecraftButton minecraftStyle='stone' onClick={() => setLogViewerOpen(false)}>
            关闭
          </MinecraftButton>
        </DialogActions>
      </Dialog>
