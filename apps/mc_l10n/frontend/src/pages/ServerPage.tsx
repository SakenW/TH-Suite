  ...
  return (
    <Box sx={{ 
      ...
      >
                  ...
                  <Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                      <Chip
                        ...
                        label={serverStatus.status === 'healthy' ? t('server.status.healthy') : 
                               serverStatus.status === 'unhealthy' ? t('server.status.unhealthy') : t('server.status.unreachable')}
                        ...
                        <ListItemIcon sx={{ minWidth: 32 }}>
                          <Box
                            sx={{
                              ...
                              height: 8,
                              borderRadius: '50%',
                              backgroundColor: pack.translation_progress >= 90 ? theme.palette.success.main :
                                             pack.translation_progress >= 70 ? theme.palette.warning.main : theme.palette.error.main,
                            }}
                          />
                        </ListItemIcon>
                        <ListItemText
                          ...
                          secondary={
                            <Box>
                              <Typography variant="caption" display="block">
                                {pack.mod_count} MODs • {pack.translation_progress.toFixed(1)}% 翻译完成
                              </Typography>
                              <Typography variant="caption" color="text.secondary">
                                支持 {pack.supported_languages.length} 种语言 • {new Date(pack.last_updated).toLocaleDateString()}
                              </Typography>
                            </Box>
                          }
                        />
                        <Chip
                          label={pack.translation_progress >= 90 ? '完成' :
                                 pack.translation_progress >= 70 ? '进行中' : '待翻译'}
                          size="small"
                          color={pack.translation_progress >= 90 ? 'success' :
                                 pack.translation_progress >= 70 ? 'warning' : 'error'}
                        />
                      </ListItem>
                    ))}
                  </List>
                  ...
                  <Box>
                    ...
                    <Box sx={{ mb: 3 }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 3, mb: 2 }}>
                        ...
                        <Box>
                          <Typography variant="h4" color="primary">
                            {Math.round((translationStats.translated_keys / translationStats.total_keys) * 100)}%
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                            {translationStats.translated_keys.toLocaleString()} / {translationStats.total_keys.toLocaleString()} 条目
                          </Typography>
                        </Box>
                      </Box>
                    </Box>
                        ...
                        <Box key={lang.code} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                          ...
                          <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
                            <LinearProgress 
                              ...
                              color={lang.progress >= 90 ? 'success' : lang.progress >= 70 ? 'warning' : 'error'}
                            />
        ...
        {/* 服务器设置对话框 */}
        <Dialog open={showServerDialog} onClose={() => setShowServerDialog(false)} maxWidth="sm" fullWidth>
          ...
          <DialogActions>
            ...
            <Button onClick={updateServerUrl} variant="contained">保存</Button>
          </DialogActions>
        </Dialog>
      </motion.div>
    </Box>
