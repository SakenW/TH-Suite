...
export default function ExportPageMinecraft() {
  ...
  const handleStartExport = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setTasks(prev => prev.map(t => 
          t.id === newTask.id 
            ? { ...t, progress, files: Math.floor(progress * 2.34), size: Math.floor(progress * 56000) }
            : t
        ));
      }
    }, 500);
  ...
  const getFormatIcon = (format: string) => {
    switch (format) {
      case 'json': return <FileJson size={20} color="#4CAF50" />;
      case 'properties': 
      case 'lang': return <FileText size={20} color="#2196F3" />;
      case 'csv': return <FileText size={20} color="#FF9800" />;
      case 'excel': return <FileText size={20} color="#4CAF50" />;
            ...
            <Box sx={{ p: 2 }}>
              ...
              <Box sx={{ mb: 3 }}>
                ...
                <RadioGroup
                  ...
                  onChange={(e) => setSelectedProfile({ ...selectedProfile, format: e.target.value as any })}
                >
                  <FormControlLabel
                    value="json"
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={(e) => setSelectedProfile({ ...selectedProfile, includeComments: e.target.checked })}
                      size="small"
                    />
                  }
                  label={
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={(e) => setSelectedProfile({ ...selectedProfile, includeMetadata: e.target.checked })}
                      size="small"
                    />
                  }
                  label={
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={(e) => setSelectedProfile({ ...selectedProfile, splitByMod: e.target.checked })}
                      size="small"
                    />
                  }
                  label={
              ...
              <FormControl fullWidth sx={{ mb: 3 }}>
                ...
                <Select
                  ...
                  onChange={(e) => setSelectedProfile({ ...selectedProfile, compression: e.target.value as any })}
                  sx={{
                    fontFamily: '"Minecraft", monospace',
                    fontSize: '14px',
                    '& fieldset': { borderRadius: 0, borderWidth: 2 }
                ...
                <AnimatePresence>
                  {tasks.map((task, index) => (
                    ...
                    >
                      ...
                      >
                        <ListItemIcon>
                          {task.status === 'completed' ? <Check size={20} color="#4CAF50" /> :
                           task.status === 'processing' ? <Play size={20} color="#2196F3" /> :
                           task.status === 'error' ? <X size={20} color="#F44336" /> :
                           <Info size={20} />}
                        </ListItemIcon>
                        <ListItemText
                          primary={
