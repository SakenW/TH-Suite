...
import { RefreshCw, Download, Database, BarChart3, ListOrdered, Link, Search, Filter, Globe, CheckCircle } from 'lucide-react';
...
const LocalDataPage: React.FC = () => {
  ...
  const filteredEntries = localEntries.filter(entry => {
    const matchesSearch = entry.source_file.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const matchesNamespace = selectedNamespace === 'all' || entry.source_context?.namespace === selectedNamespace;
    const matchesLanguage = selectedLanguage === 'all' || entry.source_lang_bcp47 === selectedLanguage;
    return matchesSearch && matchesNamespace && matchesLanguage;
  });

  const filteredPlans = mappingPlans.filter(plan => {
    const matchesSearch = plan.source_file.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const filteredQueue = outboundQueue.filter(item => {
    const matchesSearch = item.intent.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <div className="space-y-8">
          ...
          <Tabs defaultValue="entries" className="space-y-6">
            <TabsList className="grid w-full grid-cols-4 bg-white/80 backdrop-blur-sm border border-white/20 rounded-xl p-2 shadow-lg">
                <TabsTrigger value="entries" className="flex items-center space-x-2 data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all duration-200 rounded-lg">
                  <Database className="h-4 w-4" />
                  <span>本地条目</span>
                </TabsTrigger>
                <TabsTrigger value="plans" className="flex items-center space-x-2 data-[state=active]:bg-green-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all duration-200 rounded-lg">
                  <BarChart3 className="h-4 w-4" />
                  <span>映射计划</span>
                </TabsTrigger>
                <TabsTrigger value="queue" className="flex items-center space-x-2 data-[state=active]:bg-orange-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all duration-200 rounded-lg">
                  <ListOrdered className="h-4 w-4" />
                  <span>同步队列</span>
                </TabsTrigger>
                <TabsTrigger value="links" className="flex items-center space-x-2 data-[state=active]:bg-purple-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all duration-200 rounded-lg">
                ...
                <div className="space-y-3">
                  ...
                  <Select value={selectedNamespace} onValueChange={setSelectedNamespace}>
                    ...
                    <SelectContent>
                      ...
                      {Array.from(new Set(localEntries.map(e => e.source_context?.namespace).filter(Boolean))).map(ns => (
                ...
                <div className="space-y-3">
                  ...
                  <Select value={selectedLanguage} onValueChange={setSelectedLanguage}>
                    ...
                    <SelectContent>
                      ...
                      {Array.from(new Set(localEntries.map(e => e.source_lang_bcp47).filter(Boolean))).map(lang => (
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-white/20">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-semibold text-gray-800 flex items-center space-x-2">
                      ...
                      <span>本地条目数据</span>
                      <span className="text-sm font-normal text-gray-600">({filteredEntries.length} 条记录)</span>
                    </h3>
                    <Button
                      onClick={handleImportData}
                      size="sm"
                ...
                <div className="p-6">
                  {filteredEntries.length === 0 ? (
                    <div className="text-center py-16">
                      ...
                      <p className="text-gray-600 mb-6 max-w-md mx-auto">点击下方按钮导入数据开始使用本地翻译管理功能</p>
                      <Button
                        onClick={handleImportData}
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="bg-gradient-to-r from-gray-50 to-gray-100">
                            ...
                            <TableHead className="font-semibold text-gray-700 py-4">源文件</TableHead>
                            <TableHead className="font-semibold text-gray-700 py-4">命名空间</TableHead>
                            <TableHead className="font-semibold text-gray-700 py-4">语言</TableHead>
                            <TableHead className="font-semibold text-gray-700 py-4">键哈希</TableHead>
                            <TableHead className="font-semibold text-gray-700 py-4">更新时间</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {filteredEntries.map((entry, index) => (
                            <TableRow key={entry.local_id} className={`hover:bg-blue-50/50 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}`}>
                              <TableCell className="font-semibold text-blue-600 py-4">{entry.local_id}</TableCell>
                              <TableCell className="max-w-xs py-4" title={entry.source_file}>
                                <div className="truncate text-gray-800 font-medium">{entry.source_file}</div>
                              </TableCell>
                              <TableCell className="py-4">
                                <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-200 border-blue-200 font-medium px-3 py-1">{entry.source_context?.namespace}</Badge>
                              </TableCell>
                              <TableCell className="py-4">
                                <Badge className="bg-green-100 text-green-800 hover:bg-green-200 border-green-200 font-medium px-3 py-1">{entry.source_lang_bcp47}</Badge>
                              </TableCell>
                              <TableCell className="font-mono text-sm bg-gray-100 rounded-md px-3 py-2 text-gray-700">{entry.source_payload?.hash}</TableCell>
                ...
                <Card className="border-blue-200 bg-blue-50/50">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold text-blue-800">{statistics.total_entries || 0}</span>
                    </div>
                    {statistics.by_language && Object.entries(statistics.by_language).map(([lang, count]) => (
                ...
                <Card className="border-orange-200 bg-orange-50/50">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold text-orange-800">{statistics.total_snapshots || 0}</span>
                    </div>
                    {statistics.outbound_queue && Object.entries(statistics.outbound_queue).map(([state, count]) => (
                ...
                <Card className="border-purple-200 bg-purple-50/50">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold text-purple-800">{statistics.active_mappings || 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-purple-700 font-medium">未映射条目:</span>
                      <span className="text-xl font-bold text-purple-800">{statistics.unmapped_entries || 0}</span>
                    </div>
                    {statistics.link_states && Object.entries(statistics.link_states).map(([state, count]) => (
                      <div key={state} className="flex justify-between items-center">
