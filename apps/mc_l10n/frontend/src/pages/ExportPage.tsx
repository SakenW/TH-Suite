...
function ExportPage() {
  ...
  const totalKeys = languageFiles
    .filter(file => file.selected)
    .reduce((sum, file) => sum + file.keys, 0)
  const totalSize = languageFiles
    .filter(file => file.selected)
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1200, mx: 'auto' }}>
      ...
      <Typography variant='body1' color='text.secondary' sx={{ mb: 4 }}>
        {t(
          'export.description',
          'Export language files from Minecraft resource packs, mods, or custom projects. Choose your source directory, select the languages you want to export, and download them in your preferred format.',
        )}
      </Typography>
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={e =>
                        setExportOptions(prev => ({ ...prev, createZip: e.target.checked }))
                      }
                    />
                  }
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3, bgcolor: alpha(theme.palette.primary.main, 0.05) }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Typography variant='h6'>{t('export.actions.title', 'Ready to Export?')}</Typography>
              ...
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                <MinecraftButton
                  ...
                  disabled={
                    selectedCount === 0 || !outputDirectory || exportProgress.status === 'exporting'
                  }
                >
                ...
                {exportProgress.status === 'exporting' && (
                  <MinecraftButton variant='danger' size='large' onClick={handleCancelExport}>
