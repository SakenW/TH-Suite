...
const BuildPage: React.FC = () => {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        const result: BuildResult = {
          ...
          message: success ? t('build.results.success') : `${t('build.results.error')}：${t('build.messages.fileCorrupted')}`,
  ...
  const getTargetTypeColor = (type: BuildTarget['type']) => {
    switch (type) {
      case 'resourcepack': return 'primary';
      case 'datapack': return 'secondary';
  ...
  const getTargetTypeIcon = (type: BuildTarget['type']) => {
    switch (type) {
      case 'resourcepack': return <Package size={16} />;
      case 'datapack': return <FileText size={16} />;
      case 'mod': return <Settings size={16} />;
      case 'plugin': return <Upload size={16} />;
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <FormControl fullWidth size="small">
                  ...
                  <Select
                    ...
                    onChange={(e) => setBuildOptions(prev => ({ ...prev, outputFormat: e.target.value as any }))}
                  >
                  ...
                  <Box>
                    ...
                    <Box sx={{ px: 1 }}>
                      <input
                        ...
                        onChange={(e) => setBuildOptions(prev => ({ ...prev, compressionLevel: parseInt(e.target.value) }))}
                        style={{ width: '100%' }}
                      />
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      <Typography variant="caption">{t('build.options.compression.fast')}</Typography>
                      <Typography variant="caption">{t('build.options.compression.best')}</Typography>
                    </Box>
                  </Box>
                )}
                ...
                <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        onChange={(e) => setBuildOptions(prev => ({ ...prev, includeMetadata: e.target.checked }))}
                      />
                    }
                ...
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={(e) => setBuildOptions(prev => ({ ...prev, validateStructure: e.target.checked }))}
                    />
                  }
                ...
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={(e) => setBuildOptions(prev => ({ ...prev, optimizeAssets: e.target.checked }))}
                    />
                  }
                ...
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={(e) => setBuildOptions(prev => ({ ...prev, generateManifest: e.target.checked }))}
                    />
                  }
                          ...
                          <ListItemText
                            ...
                            secondary={
                              <Box>
                                ...
                                {result.duration && (
                                  <Typography variant="caption" display="block" color="text.secondary">
                                    {t('build.results.duration')}: {(result.duration / 1000).toFixed(1)}s
                                  </Typography>
                                )}
                              </Box>
                            }
