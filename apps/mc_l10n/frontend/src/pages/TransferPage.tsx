    ...
    setTransferItems(prev => prev.map(item => {
      if (selectedItems.includes(item.id)) {
        switch (action) {
          ...
          case 'cancel':
            return ['running', 'paused', 'pending'].includes(item.status) ? { ...item, status: 'cancelled' } : item;
          default:
            return item;
        }
      }
      ...
      <Card>
        <CardContent>
          ...
          <Tabs value={activeTab} onChange={(_, newValue) => setActiveTab(newValue)} sx={{ mb: 2 }}>
            ...
            <Tab label={`${t('transfer.tabs.uploads')} (${transferItems.filter(i => i.type === 'upload').length})`} />
            <Tab label={`${t('transfer.tabs.downloads')} (${transferItems.filter(i => i.type === 'download').length})`} />
              ...
              <TableBody>
                {filteredItems.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      <Chip
                        icon={item.type === 'upload' ? <Upload size={14} /> : <Download size={14} />}
