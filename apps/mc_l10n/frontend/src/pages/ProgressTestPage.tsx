  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setMockState(prev => {
        ...
        const currentFile = nextStatus === 'running' && nextProcessedFiles < mockFiles.length 
    ...
    <Box sx={{ p: 3, maxWidth: 1200, mx: 'auto' }}>
      ...
      <Paper sx={{ p: 3, mb: 4, borderRadius: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          <Chip 
            ...
            color={
              mockState.status === 'completed' ? 'success' :
              mockState.status === 'failed' ? 'error' :
              mockState.status === 'cancelled' ? 'warning' :
              mockState.status === 'running' ? 'info' : 'default'
            }
          />
          <Chip label={`进度: ${Math.round(mockState.progress)}%`} variant="outlined" />
          <Chip label={`文件: ${mockState.processed_files}/${mockState.total_files}`} variant="outlined" />
      ...
      <Paper sx={{ p: 3, borderRadius: 3, backgroundColor: 'rgba(0, 188, 212, 0.05)' }}>
        ...
        <Box component="ul" sx={{ pl: 2, color: 'text.secondary' }}>
          <li><strong>平滑动画进度条</strong>：使用 framer-motion 实现流畅的进度更新动画</li>
          <li><strong>实时文件显示</strong>：显示当前正在处理的文件，带有动态效果</li>
          <li><strong>丰富的统计信息</strong>：实时显示文件、模组、语言文件、翻译条目数量</li>
          <li><strong>自适应状态指示</strong>：根据扫描状态显示不同的颜色和图标</li>
          <li><strong>预估剩余时间</strong>：基于处理速度计算完成时间（实际扫描时）</li>
          <li><strong>错误状态显示</strong>：优雅地显示扫描错误和异常情况</li>
          <li><strong>紧凑/详细模式</strong>：支持两种显示模式，适应不同界面需求</li>
          <li><strong>交互式操作</strong>：支持取消扫描等用户操作</li>
        </Box>
      </Paper>
    </Box>
  );
