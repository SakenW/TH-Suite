...
export function ShortcutHelp() {
  ...
  const navigationShortcuts = globalShortcuts.filter(s =>
    ['返回首页', '扫描页面', '项目管理', '导出管理', '传输管理', '构建管理', '设置'].includes(
      s.description,
    ),
  )
    ...
    >
      <Table size='small'>
        <TableHead>
          <TableRow>
            <TableCell
              sx={{ fontFamily: '"Minecraft", monospace', color: minecraftColors.goldYellow }}
            >
              快捷键
            </TableCell>
            <TableCell
        ...
        <TableBody>
          {shortcuts.map((shortcut, index) => (
            <TableRow key={index} hover>
              ...
              <TableCell sx={{ color: 'text.secondary' }}>{shortcut.description}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
  ...
  return (
    <>
      ...
      >
        ...
        <DialogActions>
          <MinecraftButton minecraftStyle='stone' onClick={() => setOpen(false)}>
            关闭
          </MinecraftButton>
        </DialogActions>
      </Dialog>
