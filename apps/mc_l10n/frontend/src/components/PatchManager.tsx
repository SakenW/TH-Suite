  ...
  const handleApplyPatch = async () => {
    ...
    try {
      ...
      notification.achievement(
        '应用成功！',
        `${selectedPatch.projectName} 的翻译已成功应用`,
        {
    ...
    <Box>
      ...
      <Grid container spacing={3}>
        {patches.length === 0 ? (
          <Grid item xs={12}>
            <Alert severity="info">
              暂无可用补丁。请确保已连接到 Trans-Hub 服务器并完成扫描。
            </Alert>
          </Grid>
        ) : (
          patches.map((patch) => (
            <Grid item xs={12} md={6} lg={4} key={patch.id}>
              <MinecraftCard
                variant="chest"
                glowing={patch.downloadProgress ? true : false}
              >
                <CardContent>
