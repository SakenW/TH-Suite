...
export type FieldType = 
  ...
  | 'textarea' | 'select' | 'multiselect' | 'autocomplete'
      ...
      renderTags={(tagValue, getTagProps) =>
        tagValue.map((option, index) => (
          <Chip
            ...
            size="small"
            icon={option.icon}
            avatar={option.avatar ? <Avatar src={option.avatar} sx={{ width: 24, height: 24 }} /> : undefined}
          />
        ))
      }
    />
  ...
  const renderFileUpload = () => (
    <FormControl fullWidth={fullWidth} error={hasError}>
      ...
      >
        ...
        <Box sx={{ mb: 2 }}>
          {type === 'image' ? <Image size={48} /> : <Upload size={48} />}
        </Box>
