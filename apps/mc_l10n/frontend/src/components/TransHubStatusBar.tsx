...
import { Box, Chip, IconButton, Tooltip, Typography, Badge, CircularProgress } from '@mui/material'
import { Wifi, WifiOff, Cloud, CloudOff, RefreshCw, Upload, AlertCircle } from 'lucide-react'
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            {/* 刷新按钮 */}
            <Tooltip title='刷新状态'>
              <IconButton size='small' onClick={handleRefresh} disabled={isConnecting}>
                <RefreshCw size={14} />
              </IconButton>
            </Tooltip>
            ...
            {offlineQueueSize > 0 && isConnected && (
              <Tooltip title='同步离线队列'>
                <IconButton size='small' onClick={handleSync} color='warning'>
                  <Upload size={14} />
                </IconButton>
              </Tooltip>
            )}
