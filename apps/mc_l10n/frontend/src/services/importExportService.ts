...
class ImportExportService {
  ...
  async importData(options: ImportOptions = {}): Promise<ImportResult> {
    const { overwrite = false, merge = true, validateSchema = true, password } = options
    ...
    try {
      ...
      if (importData.data.projects) {
        const importedProjects = await this.importProjects(importData.data.projects, {
          overwrite,
          merge,
        })
      ...
      if (importData.data.translations) {
        const importedTranslations = await this.importTranslations(importData.data.translations, {
          overwrite,
          merge,
        })
  ...
  async exportProjects(projectIds: string[], options?: ExportOptions): Promise<void> {
    const projects = await Promise.all(projectIds.map(id => storageService.getProject(id)))
