...
import { ScanRequest, ScanStatus, ScanResult, ListOptions, ScanServiceInterface } from './types'
...
export class ScanService implements ScanServiceInterface {
  ...
  async startScan(request: ScanRequest): Promise<ServiceResult<{ scan_id: string }>> {
    try {
      ...
      const response = await this.apiClient.post('/api/v1/scan-project', payload, {
        timeout: 120000,
      }) // 2分钟超时
      ...
      if (response.data) {
        ...
        // 扁平格式 - 检查响应本身
        scanId =
          (response as any).scan_id ||
          (response as any).scanId ||
          (response as any).job_id ||
  ...
  async getStatus(scanId: string): Promise<ServiceResult<ScanStatus>> {
    try {
      ...
      const response = await this.apiClient.get(`/api/v1/scan-status/${scanId}`, undefined, {
        timeout: 15000,
      })
  ...
  async listScans(
    options: ListOptions = {},
  ): Promise<ServiceResult<{ scans: ScanStatus[]; pagination: any }>> {
    try {
      const params = this.buildListParams(options)
      const response = await this.apiClient.get('/api/v1/scans/history', params)
      ...
      const scans =
        response.data?.map((s: any) => ({
          ...
          total_language_files: s.total_language_files,
   ...
   * 获取最新的扫描结果
   */
  async getLatestScan(): Promise<ServiceResult<ScanResult | null>> {
    try {
      const response = await this.apiClient.get('/api/v1/scans/latest')
      ...
      if (!response.success) {
        return {
          ...
          error: {
            ...
            message: response.error?.message || '获取最新扫描结果失败',
            details: response.error,
          },
        }
      }
      ...
      return this.handleError('LATEST_SCAN_ERROR', '获取最新扫描结果时发生错误', error)
    }
  }
  ...
  private transformScanStatus(scanId: string, data: any): ScanStatus {
    ...
    const processed_files =
      data.processed_files !== undefined ? data.processed_files : progressData.processed || 0

    const total_files = data.total_files !== undefined ? data.total_files : progressData.total || 0
