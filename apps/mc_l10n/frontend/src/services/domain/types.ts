...
export interface ListOptions extends Partial<PaginationParams>, Partial<SortParams>, Partial<FilterParams> {
...
export interface ProjectServiceInterface {
  ...
  list(options?: ProjectListOptions): Promise<ServiceResult<{ projects: Project[]; pagination: any }>>;
  search(query: string, options?: ListOptions): Promise<ServiceResult<{ projects: Project[]; pagination: any }>>;
...
export interface ModServiceInterface {
  ...
  extractLanguageContent(modId: string, locale: string): Promise<ServiceResult<Record<string, string>>>;
}
...
export interface TranslationServiceInterface {
  ...
  getEntries(projectId: string, filters?: any): Promise<ServiceResult<{ entries: TranslationEntry[]; pagination: any }>>;
  updateEntry(entryId: string, translatedText: string): Promise<ServiceResult<boolean>>;
  batchUpdate(updates: Array<{ entryId: string; translatedText: string }>): Promise<ServiceResult<{ updated_count: number }>>;
}