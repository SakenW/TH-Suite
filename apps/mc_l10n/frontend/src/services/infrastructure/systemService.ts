...
import { ApiResponse, HealthCheckResponse, SystemInfo, AsyncTask, SearchResult } from '../types/api'
...
class SystemService extends BaseApiService {
   ...
   * 批量删除已完成的任务
   */
  async cleanupTasks(olderThan?: string): Promise<
    ApiResponse<{
   ...
   * 清理旧日志
   */
  async cleanupLogs(olderThan?: string): Promise<
    ApiResponse<{
   ...
   * 终止所有其他会话
   */
  async terminateOtherSessions(): Promise<
    ApiResponse<{
