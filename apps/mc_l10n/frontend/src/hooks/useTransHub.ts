...
export function useTransHub(): UseTransHubReturn {
      ...
      try {
        ...
        if (status.connected) {
          ...
          if (status.offlineQueueSize > 0) {
            notification.info('离线队列', `有 ${status.offlineQueueSize} 个待同步项目`)
          }
  ...
  const syncOfflineQueue = useCallback(async () => {
    try {
      ...
      if (result.success) {
        notification.success('同步完成', `离线队列已同步，剩余 ${result.remainingQueueSize} 项`)
      } else {
  ...
  // 上传扫描结果
  const uploadScanResults = useCallback(
    async (projectId: string, scanId: string, entries: any, metadata?: any): Promise<boolean> => {
      try {
  ...
  const getTranslationStatus = useCallback(
    async (projectId: string, targetLanguage = 'zh_cn'): Promise<TranslationStatus> => {
      try {
