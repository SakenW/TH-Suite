...
export function useProject() {
  ...
  // 提取所有useCallback到外层
  const createProject = useCallback(async (request: any) => {
    return await service.create(request);
  }, [service]);
  
  const listProjects = useCallback(async (options?: any) => {
...
export function useScan() {
  ...
  const waitForCompletion = useCallback(async (scanId: string, onProgress?: (status: any) => void) => {
