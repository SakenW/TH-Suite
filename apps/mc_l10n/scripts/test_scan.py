...
async def test_scan():
    ...
    while True:
        ...
        print(f"\r  进度: [{bar}] {progress:.1f}% - {current_file[:30]}", end='', flush=True)
    ...
    with UnifiedDatabase(str(db_path)) as db:
        ...
        if stats.get('recent_scans'):
            ...
            for scan in stats['recent_scans'][:3]:
                print(f"  - {scan['started_at']}: {scan['directory']} ({scan['status']})")
                if scan['status'] == 'completed':
                    print(f"    模组: {scan['total_mods']}, 语言文件: {scan['total_language_files']}, 键: {scan['total_keys']}")
