...
class ModEventHandler:
    ...
    def _register_handlers(self):
        ...
        bus.subscribe(TranslationConflictEvent, self.handle_translation_conflict, priority=20)
    ...
    def handle_translation_conflict(self, event: TranslationConflictEvent):
        ...
        try:
            ...
            if self.notification_repository:
                self._create_notification(
                    f"Translation conflict detected",
...
class ProjectEventHandler:
        ...
        try:
            ...
            if self.notification_repository:
                self._create_task_notification(
                    event.assignee,
                    f"New task assigned: {event.task_id}",
                    "info"
                )
    ...
    def _check_milestones(self, project_id: Optional[str]):
        """检查里程碑"""
        if project_id:
            # 实现里程碑检查逻辑
            pass
