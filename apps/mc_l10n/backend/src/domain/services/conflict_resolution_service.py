...
class TranslationConflict:
    ...
    def get_quality_diff(self) -> float:
        """获取质量差异"""
        existing_score = self.existing_entry.quality_score.value if self.existing_entry.quality_score else 0
        new_score = self.new_entry.quality_score.value if self.new_entry.quality_score else 0
    ...
    def requires_manual_review(self) -> bool:
        ...
        # 都是已批准的需要人工审核
        if self.existing_entry.status == "approved" and self.new_entry.status == "approved":
            return True
...
class ConflictResolutionService:
    ...
    def detect_conflicts(
        mod: Mod,
        new_translations: Dict[LanguageCode, List[TranslationEntry]]
                ...
                if new_entry.key in existing_map:
                    ...
                    if existing_entry.translated.value != new_entry.translated.value:
                        ...
                        if existing_entry.quality_score and new_entry.quality_score:
                            diff = abs(existing_entry.quality_score.value - new_entry.quality_score.value)
                            if diff > 0.2:
    ...
    def resolve_conflict(
        ...
        if strategy == ConflictResolutionStrategy.KEEP_NEWEST:
            ...
            # 保留质量分数最高的
            existing_score = conflict.existing_entry.quality_score.value if conflict.existing_entry.quality_score else 0
            new_score = conflict.new_entry.quality_score.value if conflict.new_entry.quality_score else 0
            ...
            # 保留更高的质量分数
            if conflict.existing_entry.quality_score and conflict.new_entry.quality_score:
                if conflict.existing_entry.quality_score.value > conflict.new_entry.quality_score.value:
    ...
    def resolve_conflicts(
        ...
        for conflict in conflicts:
            # 如果需要人工审核且策略不是强制的
            if conflict.requires_manual_review() and strategy == ConflictResolutionStrategy.MANUAL_REVIEW:
                manual_review_conflicts.append(conflict)
            else:
                resolved_entry = ConflictResolutionService.resolve_conflict(conflict, strategy)
