...
class TranslationKey:
    ...
    def __post_init__(self):
        ...
        if len(self.value) > self.MAX_KEY_LENGTH:
            raise ValueError(f"Translation key exceeds maximum length of {self.MAX_KEY_LENGTH}")
...
class TranslationText:
    ...
    def __post_init__(self):
        ...
        if len(self.value) < self.MIN_TEXT_LENGTH:
            raise ValueError(f"Translation text must be at least {self.MIN_TEXT_LENGTH} characters")
        
        if len(self.value) > self.MAX_TEXT_LENGTH:
            raise ValueError(f"Translation text exceeds maximum length of {self.MAX_TEXT_LENGTH}")
...
class QualityScore:
    ...
    def __post_init__(self):
        if not self.MIN_SCORE <= self.value <= self.MAX_SCORE:
            raise ValueError(f"Quality score must be between {self.MIN_SCORE} and {self.MAX_SCORE}")
