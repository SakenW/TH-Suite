...
from .models.mod import ModId, TranslationEntry
...
class TranslationService:
    ...
    def translate_mod(
        ...
        self.translation_repository.save_translations(
            mod_id, target_language, translations
        )
    ...
    def merge_translations(
        ...
        """
        existing = self.translation_repository.find_by_mod_and_language(
            mod_id, language
        )
    ...
    def calculate_translation_quality(self, mod_id: ModId, language: str) -> float:
        """计算翻译质量分数

        Returns:
            质量分数 (0.0-1.0)
        """
        translations = self.translation_repository.find_by_mod_and_language(
            mod_id, language
        )
...
class ConflictResolutionService:
    ...
    def detect_conflicts(
        self, mod_id: ModId, language: str, new_translations: dict[str, str]
    ) -> list[dict[str, str]]:
        """检测翻译冲突"""
        existing = self.translation_repository.find_by_mod_and_language(
            mod_id, language
        ...
        for key, new_value in new_translations.items():
            if key in existing and existing[key] != new_value:
                conflicts.append(
                    {"key": key, "existing": existing[key], "new": new_value}
                )

        return conflicts

    def resolve_conflict(
        self, conflict: dict[str, str], strategy: str = "newest"
    ) -> str:
        """解决单个冲突
        ...
        if strategy == "newest":
            ...
            return (
                conflict["new"]
                if len(conflict["new"]) > len(conflict["existing"])
    ...
    def batch_resolve(
        self, conflicts: list[dict[str, str]], strategy: str = "newest"
    ) -> dict[str, str]:
        """批量解决冲突"""
        resolved = {}
...
class TerminologyService:
    ...
    def get_term(self, term: str, language: str) -> str | None:
        """获取术语翻译"""
        return self.glossary.get(language, {}).get(term)

    def apply_terminology(self, text: str, language: str) -> str:
        """应用术语表到文本"""
        if language not in self.glossary:
            return text
    ...
    def validate_consistency(
        self, translations: dict[str, str], language: str
    ) -> list[dict[str, str]]:
        """验证术语一致性"""
        inconsistencies = []
...
class ProjectAllocationService:
    ...
    def allocate_mods_to_project(
        self, project_id: str, mod_ids: list[ModId]
    ) -> tuple[int, list[str]]:
        """分配模组到项目
    ...
    def auto_assign_tasks(
        self, project_id: str, strategy: str = "round_robin"
    ) -> dict[str, str]:
        """自动分配任务给贡献者
        ...
        if strategy == "round_robin":
            ...
            for task in pending_tasks:
                ...
                for contributor in project.contributors.values():
                    if (
                        contributor.can_translate(task.language)
                        and contributor.contribution_count > max_contributions
                    ):
