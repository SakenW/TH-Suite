...
from ..value_objects import (
    LoaderType, LanguageCode, FilePath, ContentHash,
    TranslationKey, TranslationText, QualityScore, ResourceLocation
)
...
class TranslationEntry:
    ...
    def __post_init__(self):
        ...
        if self.original.language == self.translated.language:
            raise ValueError("Original and translated text cannot have the same language")
        
        # 验证状态
        if self.status not in ["pending", "approved", "rejected"]:
            raise ValueError(f"Invalid status: {self.status}")
    
    def approve(self, reviewer: str, quality_score: Optional[QualityScore] = None):
        """批准翻译"""
        if not reviewer:
    ...
    def reject(self, reviewer: str, reason: Optional[str] = None):
        """拒绝翻译"""
        if not reviewer:
            raise ValueError("Reviewer is required")
...
class Mod:
    ...
    def deactivate(self, reason: str = None):
        ...
        self.scan_status = "failed" if self.scan_status == "scanning" else self.scan_status
        self.translation_lock = False
        self.updated_at = datetime.now()
    ...
    def can_be_deleted(self) -> bool:
        ...
        - 或者已停用的Mod可以删除
        """
        has_translations = any(len(entries) > 0 for entries in self.translations.values())
