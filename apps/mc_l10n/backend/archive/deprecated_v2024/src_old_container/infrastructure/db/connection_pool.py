...
class ConnectionPool:
    ...
    def _create_connection(self) -> ConnectionInfo:
        ...
        connection = sqlite3.connect(
            self.database_path, check_same_thread=False, timeout=self.connection_timeout
        )
            ...
            while True:
                try:
                    conn_info = self._pool.get(
                        block=True, timeout=self.connection_timeout
                    )

                    # 检查连接是否有效
                    if conn_info.is_expired(
                        self.max_age
                    ) or not self._validate_connection(conn_info):
                        # 关闭无效连接
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            stats["total_connections"] = (
                stats["pool_size"] + stats["active_connections"]
            )

            if stats["use_count_total"] > 0:
                stats["avg_wait_time"] = (
                    stats["wait_time_total"] / stats["use_count_total"]
                )
...
class AsyncConnectionPool:
    ...
    async def get_connection(self):
        ...
        # 在线程池中执行同步操作
        return await loop.run_in_executor(None, self.sync_pool.get_connection)
