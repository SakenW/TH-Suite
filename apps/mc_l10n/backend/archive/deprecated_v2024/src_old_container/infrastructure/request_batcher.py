...
class RequestBatcher:
    ...
    def register_processor(
        self, batch_key: str, processor: Callable[[list[dict]], list[Any]]
    ):
        """注册批处理器
    ...
    async def submit(
        self, batch_key: str, params: dict[str, Any], request_id: str | None = None
    ) -> Any:
        """提交请求
        ...
        batch_request = BatchRequest(
            request_id=request_id, key=batch_key, params=params, callback=future
        )
        ...
        try:
            ...
            for i, req in enumerate(batch):
                if i < len(results):
                    ...
                    req.callback.set_exception(IndexError("Result index out of range"))
...
class QueryBatcher:
    ...
    def __init__(self, connection_pool, batch_window: float = 0.05):
        ...
        self.batcher = RequestBatcher(batch_window=batch_window, max_batch_size=50)
    ...
    async def get_mod(self, mod_id: str) -> Any | None:
        """获取MOD（批处理）"""
        return await self.batcher.submit("get_mods", {"mod_id": mod_id})

    async def get_translations(
        self, mod_id: str, language: str | None = None
    ) -> list[Any]:
        """获取翻译（批处理）"""
        return await self.batcher.submit(
            "get_translations", {"mod_id": mod_id, "language": language}
        )
