...
class TranslateRequest(BaseModel):
    ...
    translator: str | None = Field(None, description="翻译者ID")
...
class CreateProjectRequest(BaseModel):
    ...
    target_languages: list[str] | None = Field(None, description="目标语言")
...
class SyncRequest(BaseModel):
    ...
    server_url: str | None = Field(None, description="服务器地址")
    conflict_strategy: str | None = Field(None, description="冲突解决策略")
...
async def scan_mods(request: ScanRequest, facade: MCL10nFacade = Depends(get_facade)):
    """扫描MOD目录并提取翻译"""
    try:
...
async def quick_scan(
    ...
    try:
        ...
        return {"success": "error" not in stats, "data": stats}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def translate_mod(
    request: TranslateRequest, facade: MCL10nFacade = Depends(get_facade)
):
    """为指定MOD添加翻译"""
    try:
...
async def batch_translate(
    request: BatchTranslateRequest, facade: MCL10nFacade = Depends(get_facade)
):
    """批量翻译多个MOD"""
    try:
...
async def create_project(
    request: CreateProjectRequest, facade: MCL10nFacade = Depends(get_facade)
):
    """创建新的翻译项目"""
    try:
...
async def get_project_status(
    project_id: str, facade: MCL10nFacade = Depends(get_facade)
):
    """获取项目详细状态"""
    try:
...
async def sync_with_server(
    request: SyncRequest, facade: MCL10nFacade = Depends(get_facade)
):
    """同步本地数据到服务器"""
    try:
        result = facade.sync_with_server(
            server_url=request.server_url, conflict_strategy=request.conflict_strategy
        )
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "translate": {"message": "Translation queued"},
            },
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_system_status(facade: MCL10nFacade = Depends(get_facade)):
    """获取系统整体状态"""
    try:
        return {
            "success": True,
