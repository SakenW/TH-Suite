...
from ..value_objects import LanguageCode, TranslationKey, TranslationText
...
class Term:
    ...
    category: str | None = None  # 分类（如：item, block, entity）
    context: str | None = None  # 使用上下文
    ...
    def get_translation(self, language: LanguageCode) -> str | None:
        """获取指定语言的翻译"""
...
class Glossary:
    ...
    def get_term(self, key: str) -> Term | None:
        """获取术语"""
...
class TerminologyService:
    ...
    def apply_terminology(
        ...
        # 按术语长度降序排序，先替换长的术语
        sorted_terms = sorted(
            glossary.terms.values(), key=lambda t: len(t.original), reverse=True
        )
    ...
    @staticmethod
    def suggest_terms(
        text: str, glossary: Glossary, language: LanguageCode, threshold: float = 0.7
        ...
        for word in words:
            ...
            for term in glossary.terms.values():
                ...
                similarity = TerminologyService._calculate_similarity(
                    word_lower, term.original.lower()
                )
                ...
                for lang, trans in translations.items():
                    if lang != base_language:
                        ...
                        for key, text in trans.items():
                            if (
                                word
                                in base_translations.get(
                                    key, TranslationText("", base_language)
                                ).value
                            ):
                                # 假设相同位置的词是翻译
    ...
    @staticmethod
    def _find_actual_translation(text: str, original: str, expected: str) -> str:
        """查找实际使用的翻译"""
