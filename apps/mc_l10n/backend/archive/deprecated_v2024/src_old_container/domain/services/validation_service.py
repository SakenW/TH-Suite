...
class ValidationService:
    ...
    def validate_translation_entry(
        cls, entry: TranslationEntry, original: str | None = None
    ) -> ValidationResult:
        """验证单个翻译条目
        ...
        if original:
            ...
            translated_format_codes = set(
                cls.FORMAT_CODE_PATTERN.findall(entry.translated)
            )
            ...
            if original.count("\n") != entry.translated.count("\n"):
                result.add_warning(f"Line break count mismatch in '{entry.key}'")
    ...
    def validate_mod_translations(
        cls, mod: Mod, language: str, source_language: str = "en_us"
    ) -> ValidationResult:
        """验证模组的所有翻译
        ...
        for translation in target_translations:
            ...
            entry_result = cls.validate_translation_entry(
                translation, source.original if source else None
            )
    ...
    def check_consistency(
        cls, translations: list[TranslationEntry]
    ) -> dict[str, list[str]]:
        """检查翻译一致性
    ...
    def check_terminology(
        cls, translation: str, terminology: dict[str, str]
    ) -> list[str]:
        """检查术语使用
    ...
    def validate_project_quality(
        cls, mod: Mod, language: str, quality_threshold: float = 0.8
    ) -> bool:
        """验证项目质量是否达标
