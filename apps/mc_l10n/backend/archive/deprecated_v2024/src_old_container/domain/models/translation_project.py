...
class Contributor:
    ...
    def can_translate(self, language: str) -> bool:
        """检查是否可以翻译指定语言"""
        return language in self.languages and self.role in [
            "translator",
            "reviewer",
            "manager",
        ]
...
class TranslationProject:
    ...
    def __init__(
        ...
        if len(self.target_languages) > self.MAX_LANGUAGES:
            raise ValueError(
                f"Cannot have more than {self.MAX_LANGUAGES} target languages"
            )
        ...
        self.completion_date: datetime | None = None
    ...
    def add_mod(self, mod_id: ModId):
        ...
        if len(self.mod_ids) >= self.MAX_MODS_PER_PROJECT:
            raise ValueError(
                f"Cannot add more than {self.MAX_MODS_PER_PROJECT} mods to a project"
            )
    ...
    def add_contributor(self, contributor: Contributor):
        ...
        if len(self.contributors) >= self.MAX_CONTRIBUTORS:
            raise ValueError(
                f"Cannot have more than {self.MAX_CONTRIBUTORS} contributors"
            )

        if contributor.user_id in self.contributors:
            raise ValueError(
                f"Contributor {contributor.user_id} is already in the project"
            )

        # 验证贡献者的语言能力与项目需求匹配
        if contributor.languages:
            matching_languages = contributor.languages.intersection(
                self.target_languages
            )
            if not matching_languages and contributor.role != "manager":
                raise ValueError(
                    "Contributor's languages don't match project's target languages"
                )
    ...
    def complete_task(self, task_id: str, quality_score: float = 1.0):
        ...
        if task.language not in self.quality_metrics:
            ...
            total_quality = (
                self.quality_metrics[task.language] * (len(completed_tasks) - 1)
                + quality_score
            )
    ...
    def complete(self):
        ...
        # 检查是否所有任务都已完成
        incomplete_tasks = [
            t for t in self.tasks if t.status not in ["completed", "cancelled"]
        ]

        if incomplete_tasks and not self.settings.allow_partial_translation:
            raise ValueError(
                f"Cannot complete project with {len(incomplete_tasks)} incomplete tasks"
            )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "in_progress_tasks": sum(
                1 for t in self.tasks if t.status == "in_progress"
            ),
    ...
    def _assign_pending_tasks_to_contributor(self, contributor: Contributor):
        ...
        pending_tasks = [
            t
            for t in self.tasks
            if t.status == "pending" and t.language in contributor.languages
        ]
    ...
    def _get_contributor_active_tasks(self, user_id: str) -> list[TranslationTask]:
        ...
        return [
            t
            for t in self.tasks
            if t.assigned_to == user_id and t.status == "in_progress"
        ]
    ...
    def _trigger_completion_check(self):
        ...
        if self.quality_metrics:
            ...
            if avg_quality >= self.settings.quality_threshold:
                if (
                    self.settings.allow_partial_translation
                    or self._all_tasks_completed()
                ):
                    self.status = ProjectStatus.COMPLETED
    ...
    def calculate_estimated_completion(self) -> datetime | None:
        """计算预估完成时间"""
        if not self.tasks:
            return None
