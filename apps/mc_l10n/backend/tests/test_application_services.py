...
from application.commands import ScanCommand, RescanCommand
from application.dto import ScanResultDTO, ModDTO
...
class TestScanService:
    ...
    def test_scan_directory_success(self, scan_service, mock_repositories):
        ...
        # 执行扫描
        with patch.object(scan_service, '_get_scannable_files', return_value=["/test/mod.jar"]):
    ...
    def test_scan_file_with_cache(self, scan_service, mock_repositories):
        ...
        # 扫描文件（应该跳过）
        result = scan_service.scan_file("/test/mod.jar", force=False)
    ...
    def test_scan_file_force_rescan(self, scan_service, mock_repositories):
        ...
        # 强制重新扫描
        result = scan_service.scan_file("/test/mod.jar", force=True)
...
class TestDTOs:
    ...
    def test_scan_result_dto(self):
        ...
        assert dto.success == False  # 因为有失败文件
