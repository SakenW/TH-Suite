...
# 最大递归深度常量
_MAX_RECURSION_DEPTH = 1000


def _assert_i_json_compat(value: Any, path: str = "$", depth: int = 0) -> None:
    """
    ...
    Args:
        value: 要检查的值
        path: 当前值在对象树中的路径（用于错误报告）
        depth: 当前递归深度
        
    Raises:
        CanonicalizationError: 当不符合 I-JSON 规范或递归深度超限时
    """
    # 检查递归深度
    if depth > _MAX_RECURSION_DEPTH:
        raise CanonicalizationError(
            f"I-JSON 校验失败: 递归深度超过限制 {_MAX_RECURSION_DEPTH}，路径: '{path}'"
        )
    ...
    if isinstance(value, dict):
        for k, v in value.items():
            ...
            _assert_i_json_compat(v, f"{path}.{k}", depth + 1)
        return
    if isinstance(value, list):
        for i, v in enumerate(value):
            _assert_i_json_compat(v, f"{path}[{i}]", depth + 1)
        return
