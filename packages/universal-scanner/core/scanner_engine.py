...
class UniversalScannerEngine(ScannerInterface):
    ...
    async def _detect_game_type(self, path: Path) -> str | None:
        """自动检测游戏类型"""
        # 让每个注册的处理器尝试识别
    ...
    async def scan(
        self, request: ScanRequest, progress_callback: ProgressCallback | None = None
        ...
        try:
            ...
            discovered_files = await self._discover_files(
                request.target_path, request.scan_options
            )
            ...
            logger.info(
                f"✅ Scan completed: {request.scan_id} - {len(all_content_items)} items discovered"
            )
            yield result
    ...
    async def _discover_files(
        self, target_path: Path, scan_options: dict
    ) -> list[Path]:
        """发现需要处理的文件"""
        discovered_files = []
    ...
    def _register_progress_callback(
        self, scan_id: str, callback: ProgressCallback
    ) -> None:
        """注册进度回调"""
    ...
    async def get_scan_history(
        self, game_type: str | None = None, limit: int = 50
    ) -> list[dict]:
        """获取扫描历史"""
        # 这里可以实现从数据库获取扫描历史的逻辑
        # 暂时返回空列表
