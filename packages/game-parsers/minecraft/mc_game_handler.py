...
from core.scanner_interface import (
    GameSpecificHandler, ContentItem, ContentType
)
...
class MinecraftGameHandler(GameSpecificHandler):
    ...
    async def _detect_modpack_info(self, path: Path) -> Optional[Dict[str, Any]]:
        """检测组合包信息"""
        if not path.is_dir():
            return None
                ...
                with open(manifest_path, 'r', encoding='utf-8') as f:
                    ...
                    if manifest.get('manifestType') == 'minecraftModpack':
                        ...
                        return {
                            ...
                            "mod_loader": mod_loaders[0].get('id', 'unknown').split('-')[0] if mod_loaders else 'unknown',
            ...
            try:
                with open(modrinth_path, 'r', encoding='utf-8') as f:
                    ...
                    return {
                        ...
                        "mod_loader": next((k for k in dependencies.keys() if k != 'minecraft'), 'unknown'),
                    ...
                    with open(instance_cfg_path, 'r', encoding='utf-8') as f:
                        for line in f:
                            ...
                            if line.startswith('name='):
                                ...
                                multimc_info['minecraft_version'] = line.split('=', 1)[1]
    ...
    async def _detect_mod_info(self, jar_path: Path) -> Optional[Dict[str, Any]]:
        """检测单个MOD信息"""
        try:
                ...
                for loader, metadata_files in self.mod_metadata_files.items():
                    for metadata_file in metadata_files:
                        if metadata_file in file_list:
                            mod_info = await self._parse_mod_metadata(jar, metadata_file, loader)
                            if mod_info:
                                mod_info.update({
    ...
    async def _parse_mod_metadata(self, jar: zipfile.ZipFile, metadata_file: str, loader: str) -> Optional[Dict[str, Any]]:
        """解析MOD元数据"""
        try:
                    ...
                    if loader == "fabric":
                        ...
                        if 'quilt_loader' in data:
                            ...
                            return {
                                ...
                                "authors": metadata.get('contributors', {}).get('author', [])
                            }
    ...
    async def _detect_dev_environment(self, path: Path) -> Optional[Dict[str, Any]]:
        """检测MOD开发环境"""
        # 检查Gradle构建文件
        if (path / "build.gradle").exists() or (path / "build.gradle.kts").exists():
            ...
            for gradle_file in gradle_files:
                try:
                    ...
                    if any(keyword in content.lower() for keyword in ['minecraft', 'forge', 'fabric', 'quilt']):
                        return {
                            "build_system": "gradle",
        ...
        try:
            with zipfile.ZipFile(jar_path, 'r') as jar:
                ...
                metadata.update({
                    ...
                    "language_files_count": len([f for f in file_list if self._is_language_file(f)]),
                    "metadata_files": [f for f in file_list if any(mf in f for loader_files in self.mod_metadata_files.values() for mf in loader_files)]
                })
