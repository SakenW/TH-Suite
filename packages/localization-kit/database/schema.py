...
from sqlalchemy import (
    ...
    ForeignKey, Index, UniqueConstraint, JSON, BLOB
)
...
class ArtifactTable(Base):
    ...
    artifact_type = Column(String, nullable=False)  # mod_jar | modpack_dir | resource_pack | data_pack
    ...
    # 关系
    containers = relationship("ContainerTable", back_populates="artifact", cascade="all, delete-orphan")
...
class ContainerTable(Base):
    ...
    language_files = relationship("LanguageFileTable", back_populates="container", cascade="all, delete-orphan")
    ...
    __table_args__ = (
        ...
        UniqueConstraint('artifact_id', 'mod_id', 'version', name='uq_container_identity'),
    )
...
class LanguageFileTable(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('container_id', 'locale', 'namespace', 'file_path', name='uq_language_file'),
    )
...
class SerializationProfileTable(Base):
    ...
    # 索引
    __table_args__ = (
        Index('idx_serialization_format', 'format'),
    )
...
class PatchSetTable(Base):
    ...
    # 关系
    patch_items = relationship("PatchItemTable", back_populates="patch_set", cascade="all, delete-orphan")
...
class PatchItemTable(Base):
    ...
    namespace = Column(String, nullable=False)
    locale = Column(String, nullable=False)
    policy = Column(String, nullable=False)  # overlay | replace | merge | create_if_missing
    expected_blob_hash = Column(String(64))
    expected_entry_count = Column(Integer)
    serializer_profile_id = Column(String, ForeignKey('serialization_profiles.profile_id'))
    ...
    __table_args__ = (
        ...
        UniqueConstraint('patch_set_id', 'target_container_id', 'namespace', 'locale', name='uq_patch_item'),
    )
...
class ApplyRunTable(Base):
    ...
    # 关系
    results = relationship("ApplyResultTable", back_populates="apply_run", cascade="all, delete-orphan")
...
class ApplyResultTable(Base):
    ...
    patch_item_id = Column(String, ForeignKey('patch_items.patch_item_id'), nullable=False)
    ...
    # 索引
    __table_args__ = (
        Index('idx_apply_result_status', 'status'),
    )
...
class QualityCheckTable(Base):
    ...
    entry_key = Column(String, nullable=False)
    check_type = Column(String, nullable=False)  # placeholder | format | color_code | length
...
class LocaleAliasTable(Base):
    ...
    # 索引
    __table_args__ = (
        Index('idx_locale_bcp47', 'bcp47_locale'),
    )