...
class TransHubClient:
    ...
    async def _request(self, method: str, endpoint: str, **kwargs) -> tuple[bool, Any]:
        """
        发送 HTTP 请求
    ...
    async def create_project(
        ...
        payload = {
            ...
            "metadata": {"platform": "minecraft", "created_by": "localization_kit"},
        }

        success, data = await self._request("POST", "/api/v1/projects", json=payload)
    ...
    async def get_project(self, project_id: str) -> tuple[bool, dict | None]:
        """
        获取项目信息
        ...
        Returns:
            (成功标志, 项目信息)
        """
        success, data = await self._request("GET", f"/api/v1/projects/{project_id}")
    ...
    async def upload_blob(
        ...
        success, data = await self._request(
            "POST", f"/api/v1/projects/{project_id}/blobs", json=payload
        )
    ...
    async def request_translation(
        ...
        payload = {
            ...
            "keys": keys,
            "engine": engine,
            "options": {"quality_check": True, "preserve_placeholders": True},
        }

        success, data = await self._request(
            "POST", f"/api/v1/projects/{project_id}/translate", json=payload
        )
    ...
    async def get_translation_status(
        ...
        """
        success, data = await self._request(
            "GET", f"/api/v1/projects/{project_id}/translate/{task_id}"
        )
    ...
    async def download_patch_set(
        self, project_id: str, locale: str, since_timestamp: str | None = None
    ) -> tuple[bool, dict | None]:
        """
        下载补丁集
        ...
        success, data = await self._request(
            "GET", f"/api/v1/projects/{project_id}/patches", params=params
        )
    ...
    async def download_translated_content(
        self, project_id: str, namespace: str, locale: str
        ...
        """
        success, data = await self._request(
            "GET", f"/api/v1/projects/{project_id}/content/{namespace}/{locale}"
        )
    ...
    async def sync_status(self, project_id: str) -> tuple[bool, dict | None]:
        """
        同步项目状态
        ...
        """
        success, data = await self._request(
            "GET", f"/api/v1/projects/{project_id}/status"
        )
    ...
    async def report_apply_result(
        self, project_id: str, patch_set_id: str, results: list[dict]
    ) -> tuple[bool, str | None]:
        """
        报告补丁应用结果
        ...
        success, data = await self._request(
            "POST", f"/api/v1/projects/{project_id}/apply-reports", json=payload
        )
