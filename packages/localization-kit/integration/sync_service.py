...
from ..models import Container, PatchPolicy, PatchSet
...
class SyncService:
    ...
    def __init__(
        self,
        session: Session,
        transhub_client: TransHubClient | None = None,
    ...
    async def sync_project(
        ...
        async with self.client:
            ...
            # 5. 下载补丁
            patch_results = await self._download_patches(project_id, target_locales)
    ...
    async def _create_project(self, container: Container) -> tuple[bool, str | None]:
        """创建 Trans-Hub 项目"""
        project_name = (
            f"MC_{container.display_name}_{datetime.now().strftime('%Y%m%d')}"
        )
    ...
    async def _upload_content(
        self, project_id: str, containers: list[Container], incremental: bool
    ) -> dict[str, Any]:
        """上传内容到 Trans-Hub"""
        upload_stats = {
            "blob_count": 0,
    ...
    async def _request_translations(
        self, project_id: str, target_locales: list[str]
    ) -> list[str]:
        """请求翻译任务"""
        task_ids = []
    ...
    async def _download_patches(
        ...
        """下载补丁"""
        patch_stats = {"patch_sets": [], "patch_items": [], "total_entries": 0}
        ...
        for locale in target_locales:
            ...
            success, patch_data = await self.client.download_patch_set(
                project_id=project_id, locale=locale, since_timestamp=since_timestamp
            )
    ...
    async def _import_patch_set(self, patch_data: dict, locale: str) -> PatchSet | None:
        """导入补丁集"""
        try:
            # 创建补丁集
            ...
            for item_data in patch_data.get("patch_items", []):
                ...
                passed, results = self.quality_gate.validate_batch(
                    item_data.get("content", {}), item_data.get("source_content", {})
                )

                if not passed:
                    logger.warning(
                        f"补丁项质量检查失败: {item_data.get('namespace')}:{locale}"
                    )
                    continue
