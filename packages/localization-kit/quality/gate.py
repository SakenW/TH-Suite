...
from ..models import Blob, PatchItem
...
class QualityGate:
    ...
    def validate_entry(
        ...
        if validators:
            selected_validators = [
                self.validators[name] for name in validators if name in self.validators
            ]
        else:
    ...
    def validate_patch_item(
        self, patch_item: PatchItem, source_blob: Blob | None = None
    ) -> tuple[bool, list[ValidationResult]]:
        """
        验证补丁项
    ...
    def get_statistics(self) -> dict[str, any]:
        ...
        if self.session:
            ...
            total_checks = (
                self.session.query(func.count(QualityCheckTable.check_id)).scalar() or 0
            )

            failed_checks = (
                self.session.query(func.count(QualityCheckTable.check_id))
