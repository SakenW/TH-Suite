...
class AuditLogger:
    ...
    def log_scan(
        self, action: str, path: str, result: str = "success", stats: dict | None = None
    ) -> AuditEvent:
        """记录扫描事件"""
        event_type = {
    ...
    def log_quality(
        ...
        """记录质量检查事件"""
        event_type = (
            AuditEventType.QUALITY_PASSED if passed else AuditEventType.QUALITY_FAILED
        )
    ...
    def export(self, format: str = "json") -> str:
        ...
        if format == "json":
            return json.dumps(
                [event.to_dict() for event in self.events], ensure_ascii=False, indent=2
            )
        elif format == "csv":
            import csv
            import io
