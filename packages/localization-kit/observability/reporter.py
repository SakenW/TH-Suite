...
class ReportGenerator:
    ...
    def generate_apply_report(
        ...
        report = ApplyReport(
            ...
            metadata={
                ...
                'conflict_rate': conflict_entries / total_entries if total_entries > 0 else 0,
                'average_duration_ms': sum(i.duration_ms for i in items) / len(items) if items else 0
            }
        )
    ...
    def generate_scan_report(
        ...
        if self.metrics:
            report['performance'] = {
                ...
                'throughput': self.metrics.metrics.get('scan_file_count', {}).get_rate()
            }
    ...
    def generate_quality_report(
        ...
        if total > 0:
            ...
            report['summary']['failure_rate'] = report['summary']['failed_entries'] / total
    ...
    def generate_audit_report(
        ...
        # 查询关键事件
        from .audit import AuditLevel, AuditEventType
        ...
        # 查询失败操作
        failed_ops = [
            e for e in self.audit.query(start_time=start_time, end_time=end_time, limit=1000)
            if e.result == 'failed'
        ][:50]
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        # 时延分析
        latency_metrics = ['scan_duration', 'patch_apply_duration', 'api_request_duration']
        for metric_name in latency_metrics:
            if metric_name in self.metrics.metrics:
                ...
                report['latencies'][metric_name] = {
                    ...
                    'max': max([v.value for v in metric.values]) if metric.values else None
                }
        ...
        if 'memory_usage' in self.metrics.metrics:
            ...
            report['resource_usage']['memory'] = {
                ...
                'max': max([v.value for v in memory_metric.values]) if memory_metric.values else None,
    ...
    def _save_report(
        ...
        if hasattr(report, 'report_id'):
            ...
            filename = f"{report_type.value}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            for file_path in sorted(dir_path.glob("*.json"), reverse=True)[:limit]:
                try:
                    ...
                    reports.append({
                        ...
                        'time': data.get('report_time') or data.get('apply_time') or data.get('scan_time'),
