...
class PatchItem:
    ...
    expected_blob_hash: str | None = None  # 期望的内容哈希
    expected_entry_count: int | None = None  # 期望的条目数
    serializer_profile_id: str | None = None  # 序列化配置
    target_member_path: str | None = None  # JAR 内的目标路径
    upstream_anchor_blob: str | None = None  # 上游锚定 Blob（用于三方合并）
    ...
    def set_content(self, entries: dict[str, str]) -> None:
        ...
        self.expected_blob_hash = hashlib.sha256(
            canonical_json.encode("utf-8")
        ).hexdigest()
        self.expected_entry_count = len(entries)

    def validate_preconditions(self, current_hash: str | None) -> bool:
        """
        验证前置条件
...
class PatchSet:
    ...
    @classmethod
    def create(
        cls, name: str, description: str | None = None, version: str | None = None
    ) -> "PatchSet":
        """创建补丁集"""
        return cls(
    ...
    def get_patch_item(self, patch_item_id: str) -> PatchItem | None:
        """获取特定补丁项"""
        for item in self.patch_items:
            if item.patch_item_id == patch_item_id:
                return item
    ...
    def get_items_by_locale(self, locale: str) -> list[PatchItem]:
        """获取特定语言的所有补丁项"""
        return [item for item in self.patch_items if item.locale == locale]
