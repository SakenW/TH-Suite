...
class WritebackStrategy(ABC):
    ...
    @abstractmethod
    def apply(
        self, patch_item: PatchItem, target_path: Path, **kwargs
    ) -> WritebackResult:
        """
        应用补丁
        ...
        try:
            ...
            if patch_item.expected_blob_hash:
                ...
                canonical_json = json.dumps(
                    final_content, sort_keys=True, ensure_ascii=False
                )
...
class JarModifyStrategy(WritebackStrategy):
    ...
    def apply(
        ...
        try:
            # 创建备份
            backup_name = (
                f"{target_path.stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jar"
            )
