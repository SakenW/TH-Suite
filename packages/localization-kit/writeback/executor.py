...
from ..models import PatchSet, PatchItem, PatchPolicy, Container
from ..database.schema import (
    WritebackPlanTable, ApplyRunTable, ApplyResultTable,
    ContainerTable, ArtifactTable
)
from ..services.patch_service import PatchService
from .strategy import (
    WritebackStrategy, WritebackResult,
    OverlayStrategy, JarModifyStrategy, DirectoryWriteStrategy
)
...
class WritebackExecutor:
    ...
    def __init__(
        self,
        session: Session,
        resource_pack_dir: Optional[Path] = None,
        ...
        # 当前执行状态
        self.current_run: Optional[ApplyRunTable] = None
    ...
    def execute_plan(
        ...
        """
        # 获取计划
        plan = self.session.query(WritebackPlanTable).filter_by(
            plan_id=plan_id
        ).first()
        ...
        for patch_item in patch_set.patch_items:
            ...
            try:
                # 获取目标容器和路径
                container_info = self._get_container_info(patch_item.target_container_id)
                ...
                if result.success:
                    success_count += 1
                    logger.info(f"[{trace_id}] 补丁应用成功: {patch_item.patch_item_id}")
                else:
                    failed_count += 1
        ...
        logger.info(
            f"[{trace_id}] 执行完成: 成功 {success_count}, 失败 {failed_count}"
        )
    ...
    def _get_container_info(self, container_id: str) -> Optional[Dict]:
        """获取容器信息"""
        container = self.session.query(ContainerTable).filter_by(
        ...
        artifact = self.session.query(ArtifactTable).filter_by(
    ...
    def _select_strategy(self, patch_item: PatchItem, target_path: Path) -> WritebackStrategy:
        """选择合适的回写策略"""
    ...
    def rollback_run(self, run_id: str) -> Tuple[bool, List[str]]:
        ...
        """
        # 获取执行记录
        run = self.session.query(ApplyRunTable).filter_by(
            run_id=run_id
        ).first()
        ...
        # 获取所有结果
        results = self.session.query(ApplyResultTable).filter_by(
            run_id=run_id
        ).all()
