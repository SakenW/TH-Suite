...
from ..database.schema import (
    BlobTable, LanguageFileTable, EntryCurrentTable
)
...
class BlobService:
    ...
    def store_blob(self, entries: Dict[str, str]) -> Tuple[Blob, bool]:
        ...
        # 检查是否已存在
        existing = self.session.query(BlobTable).filter_by(
        ...
        logger.info(f"新 Blob 已存储: {blob.blob_hash[:8]}... ({blob.entry_count} 条目)")
        return blob, True
    ...
    def _store_entries(self, blob: Blob) -> None:
        ...
        for key, value in blob.entries.items():
            ...
            # 检查是否已存在
            existing = self.session.query(EntryCurrentTable).filter_by(
                entry_key=entry_key
            ).first()
    ...
    def get_blob(self, blob_hash: str) -> Optional[Blob]:
        ...
        Returns:
            Blob 对象或 None
        """
        blob_table = self.session.query(BlobTable).filter_by(
            blob_hash=blob_hash
        ).first()
    ...
    def find_similar_blobs(self, blob: Blob, threshold: float = 0.8) -> List[Tuple[Blob, float]]:
        """
        查找相似的 Blob
        ...
        # 查询所有 Blob
        all_blobs = self.session.query(BlobTable).filter(
            BlobTable.blob_hash != blob.blob_hash
        ).all()
    ...
    def diff_blobs(self, blob_hash1: str, blob_hash2: str) -> Dict[str, Dict[str, Optional[str]]]:
        """
        比较两个 Blob 的差异
    ...
    def get_blob_references(self, blob_hash: str) -> List[LanguageFile]:
        ...
        # 查询引用此 Blob 的语言文件
        file_tables = self.session.query(LanguageFileTable).filter_by(
    ...
    def update_references(self, blob_hash: str, file_ids: List[str]) -> None:
        ...
        for file_id in file_ids:
            lang_file = self.session.query(LanguageFileTable).filter_by(
                file_id=file_id
            ).first()
    ...
    def garbage_collect(self, dry_run: bool = True) -> List[str]:
        ...
        """
        # 查找无引用的 Blob
        orphaned_blobs = self.session.query(BlobTable).filter(
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'total_entries': self.session.query(func.count(EntryCurrentTable.entry_key)).scalar(),
            'total_size': self.session.query(func.sum(BlobTable.size)).scalar() or 0,
            'avg_entry_count': self.session.query(func.avg(BlobTable.entry_count)).scalar() or 0,
            'max_entry_count': self.session.query(func.max(BlobTable.entry_count)).scalar() or 0,
            'orphaned_count': len(self.garbage_collect(dry_run=True))
        }
        
        # 去重率计算
        total_references = self.session.query(func.count(LanguageFileTable.file_id)).filter(
            LanguageFileTable.content_hash.isnot(None)
        ).scalar()
