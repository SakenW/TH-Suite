...
from ..models import (
    PatchSet, PatchItem, PatchPolicy, PatchStatus,
    Container, LanguageFile, Blob
)
from ..database.schema import (
    PatchSetTable, PatchItemTable, ContainerTable,
    LanguageFileTable, BlobTable
)
...
class PatchService:
    ...
    def generate_patch_item(
        ...
        # 获取当前语言文件（如果存在）
        current_file = self.session.query(LanguageFileTable).filter_by(
            container_id=container_id,
            locale=locale,
            namespace=namespace
        ).first()
    ...
    def add_patch_item_to_set(
        ...
        """
        # 验证补丁集存在
        patch_set_table = self.session.query(PatchSetTable).filter_by(
        ...
        logger.info(f"补丁项已添加到补丁集: {patch_item.patch_item_id} -> {patch_set_id}")
    
    def get_patch_set(self, patch_set_id: str) -> Optional[PatchSet]:
        """
        获取补丁集
        ...
        # 加载补丁项
        patch_items = self.session.query(PatchItemTable).filter_by(
            patch_set_id=patch_set_id
        ).all()
    ...
    def export_patch_set(self, patch_set_id: str) -> Optional[Dict]:
        """
        导出补丁集为 JSON 清单
