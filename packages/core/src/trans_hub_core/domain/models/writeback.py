...
class WritebackExecutor(ABC):
    ...
    def create_backup(
        self, target: Path, backup_type: BackupType = BackupType.FULL
    ) -> BackupInfo:
...
class OverlayWritebackExecutor(WritebackExecutor):
    ...
    def __init__(self, resourcepacks_dir: Path | None = None):
        ...
        try:
            ...
            with zipfile.ZipFile(plan.target_path, "r") as old_jar:
                with zipfile.ZipFile(temp_jar, "w", zipfile.ZIP_DEFLATED) as new_jar:
                    ...
                    lang_content = json.dumps(
                        plan.content, indent=2, ensure_ascii=False
                    )
...
class WritebackManager:
    ...
    def rollback(self, result_or_id: ApplyResult | str) -> bool:
