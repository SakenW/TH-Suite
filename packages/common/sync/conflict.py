...
class TimestampResolver(ConflictResolver):
    ...
    def can_resolve(self, conflict: ConflictItem) -> bool:
        """检查是否有时间戳信息"""
        return (
            conflict.local_timestamp is not None
            and conflict.remote_timestamp is not None
        )
...
class ThreeWayMergeResolver(ConflictResolver):
    ...
    def _merge_dicts(self, conflict: ConflictItem) -> ConflictResolution:
        ...
        return ConflictResolution(
            ...
            notes="Dictionary merged with conflicts"
            if conflicts_found
            else "Dictionary merged successfully",
        )
...
class FieldLevelResolver(ConflictResolver):
    ...
    def resolve(self, conflict: ConflictItem) -> ConflictResolution:
        ...
        # 处理未指定策略的字段
        all_fields = set(conflict.local_value.keys()) | set(
            conflict.remote_value.keys()
        )
        for field in all_fields:
            if field not in merged:
                merged[field] = conflict.local_value.get(
                    field, conflict.remote_value.get(field)
                )
                confidence_scores.append(0.5)
            ...
            if resolver.can_resolve(conflict):
                try:
                    ...
                    if (
                        ...
                        and not resolution.manual_review_required
                    ):
                        return resolution
