...
class SyncClient(ABC):
    ...
    @abstractmethod
    def get_remote_items(self, sync_token: Optional[str] = None) -> Tuple[List[SyncItem], Optional[str]]:
    ...
    def sync(self) -> Dict[str, Any]:
        ...
        for item in to_push:
            try:
                if self.push_item(item):
                    ...
                    self.sync_state.pending_items.remove(item.id) if item.id in self.sync_state.pending_items else None
                else:
        ...
        logger.info(f"Sync completed: pushed={results['pushed']}, pulled={results['pulled']}, "
                   f"conflicts={results['conflicts']}, errors={results['errors']}")
        
        return results
    
    def _handle_conflict(self, item: SyncItem) -> Optional[SyncItem]:
        """处理冲突"""
...
class DeltaSync(SyncClient):
    ...
    def get_delta(self, current_items: List[SyncItem], snapshot: Dict[str, str]) -> Dict[str, str]:
        """获取增量
        ...
        for item in current_items:
            ...
            current_hash = self.calculate_hash(item.local_data if item.local_data else item.remote_data)
