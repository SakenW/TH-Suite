...
class PipelineStage(ABC):
    ...
    @abstractmethod
    def process(self, context: PipelineContext) -> Optional[PipelineContext]:
        """处理上下文"""
        pass
    
    def execute(self, context: PipelineContext) -> Optional[PipelineContext]:
        """执行阶段处理"""
        try:
            # 处理当前阶段
    ...
    def process(self, context: PipelineContext) -> Optional[PipelineContext]:
        ...
        try:
            if context.file_path.suffix in ['.jar', '.zip']:
                ...
                # 文本文件
                with open(context.file_path, 'r', encoding=self.encoding) as f:
                    context.set_data('content', f.read())
...
class ScanPipeline:
    ...
    def process(self, file_path: Path) -> Optional[PipelineContext]:
        """处理文件"""
        if not self.stages:
    ...
    def process_batch(self, file_paths: List[Path], max_workers: int = 4) -> List[PipelineContext]:
        """批量处理"""
        results = []
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_path = {
                executor.submit(self.process, path): path
...
class AsyncPipeline:
    ...
    def get_result(self, timeout: Optional[float] = None) -> Optional[PipelineContext]:
        """获取处理结果"""
        try:
