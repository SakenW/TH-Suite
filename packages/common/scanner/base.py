...
class BaseScanner(ABC):
    ...
    def scan(self) -> Generator[ScanResult, None, None]:
        ...
        logger.info(f"Scan completed: {self.progress.processed_files} processed, "
                   f"{self.progress.failed_files} failed")
    
    def _scan_file_safe(self, file_path: Path) -> Optional[ScanResult]:
        """安全地扫描文件（捕获异常）"""
        try:
...
class IncrementalScanner(BaseScanner):
    ...
    def _is_file_changed(self, file_path: Path) -> bool:
        ...
        try:
            stat = file_path.stat()
            # 比较修改时间和大小
            if (stat.st_mtime != cached_info.get('mtime') or
                stat.st_size != cached_info.get('size')):
                return True
        except Exception:
            return True
    ...
    @abstractmethod
    def _do_scan_file(self, file_path: Path) -> Optional[ScanResult]:
        """实际执行文件扫描（子类实现）"""
        pass
